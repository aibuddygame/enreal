...
const OUTCOMES = [
    {
        title: '工具 → 策略',
        desc: '理解AI如何幫公司賺錢、慳成本、提升效率。從操作者變成策略設計者。',
        image: 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?q=80&w=800&auto=format&fit=crop'
    },
    {
        title: '執行 → 設計',
        desc: '建立Marketing AI流程、文件與報告自動化、數據分析與決策框架。',
        image: 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?q=80&w=800&auto=format&fit=crop'
    },
    {
        title: '員工 → 被看見',
        desc: '完成一個真實AI MVP項目，用成果講價，成為公司離唔開嘅人。',
...
const CURRICULUM = [
    {
        phase: 'Phase 1',
        title: 'AI思維 + 工具掌握',
        subtitle: 'Sessions 1-4',
        desc: '聚焦：用AI幫公司賺錢／慳時間\n\n內容：常用工具、企業落地實踐（Marketing/Operations）、解難框架\n\n成果：發展出個人AI提案',
        learn: ['AI點樣幫公司賺錢／慳成本／提升效率', 'Marketing AI流程設計', '文件與報告自動化', '數據分析與決策框架建立'],
    },
    {
        phase: 'Phase 2',
        title: 'MVP實踐',
        subtitle: 'Sessions 5-6',
        desc: '聚焦：從「諗」到「做」\n\n內容：將提案轉化為MVP、模擬場景、導師指導優化\n\n成果：完成一個可展示嘅真實項目',
        learn: ['將AI提案轉化為可行MVP', '模擬真實企業場景', '導師一對一指導優化', '項目展示技巧'],
    },
    {
        phase: 'Phase 3',
        title: '高管展示 🔥',
        ...
        learn: ['向C-Level匯報嘅展示技巧', '真實CEO/MD面前展示成果', '即場反饋與優化建議', '建立高層人脈網絡'],
    }
]
    ...
    return (
        <div ref={mainRef} style={{ background: T.bg, color: T.text, fontFamily: 'Inter, sans-serif', overflowX: 'hidden' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
                ...
                <Footer navLinks={[{ label: '問題', id: 'problem' }, { label: '解決方案', id: 'solution' }, { label: '成果', id: 'outcomes' }, { label: '課程大綱', id: 'curriculum' }, { label: '定價', id: 'pricing' }]} brandText="成為公司離唔開嘅人" accentColor="#059669" />
            </div>
        </div>
    )
}
...
function Hero() {
    ...
    useEffect(() => {
        ...
        if (phase === 'typing') {
            if (display.length < target.length) schedule(() => setDisplay(target.slice(0, display.length + 1)), 60)
            else setPhase('paused')
        }
        if (phase === 'paused') schedule(() => setPhase('deleting'), 1500)
        if (phase === 'deleting') {
            if (display.length > 0) schedule(() => setDisplay(d => d.slice(0, -1)), 30)
            else schedule(() => { setPhraseIdx(i => (i + 1) % PHRASES.length); setPhase('typing') }, 220)
        }
    ...
    return (
        <section id="hero" style={{ height: '100dvh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 5vw', position: 'relative' }}>
            <div style={{ maxWidth: 900, marginTop: '10vh', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', width: '100%' }}>
                <p className="h-rev" style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.7rem', letterSpacing: '0.22em', color: '#EF4444', marginBottom: '2.5rem', opacity: 0.85, textTransform: 'uppercase' }}>【開場3秒 Hook】</p>
                ...
                <p className="h-rev" style={{ fontFamily: 'Manrope, sans-serif', fontSize: 'clamp(1rem, 1.8vw, 1.1rem)', color: T.muted, lineHeight: 1.75, maxWidth: 600, margin: '2rem auto 2.5rem' }}>
                    大部分人學AI，只停留喺「問問題、寫內容、做分析」<br />但企業真正需要係：可以令AI穩定產出結果嘅人
                </p>
                <div className="h-rev" style={{ display: 'flex', gap: '1rem', alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap' }}>
                    <button onClick={() => go('pricing')} style={{ background: '#EF4444', color: '#FFF', padding: '1rem 2rem', border: 'none', borderRadius: 999, fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', transition: 'transform 0.2s' }} onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.03)'} onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>立即報名 <ArrowRight size={16} /></button>
                    <button onClick={() => go('problem')} style={{ background: 'transparent', color: T.text, padding: '1rem 1.5rem', border: `1px solid ${T.border}`, borderRadius: 999, fontSize: '0.9rem', fontWeight: 500, cursor: 'pointer', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(0,0,0,0.05)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>了解更多</button>
                </div>
            </div>
        </section>
    )
        ...
        <section id="problem" className="sec-rev" style={{ padding: '8rem 5vw', background: T.surface }}>
            <div style={{ maxWidth: 900, margin: '0 auto' }}>
                <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
                    <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', letterSpacing: '0.1em', color: '#EF4444', marginBottom: '1.5rem', textTransform: 'uppercase' }}>// 主故事</p>
                    <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', fontWeight: 600, letterSpacing: '-0.03em', lineHeight: 1.2 }}>
                        你應該好熟呢個畫面——<br /><span style={{ color: T.muted }}>老闆喺會議室講：「我哋要做AI轉型」</span><br />然後望住你
                    </h2>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem', marginTop: '3rem' }}>
                    {['你卡住咗', '老闆失去信心', '你仍然喺原位'].map((title, i) => (
                        <div key={i} style={{ padding: '2rem', background: T.bg, borderRadius: '1rem', border: `1px solid ${T.border}` }}>
                            ...
                            <p style={{ color: T.muted, fontSize: '0.95rem', lineHeight: 1.6 }}>
                                {i === 0 ? '當你要寫proposal向C-Level解釋點樣真正落地，你卡住。問題唔係你唔夠努力，係你缺咗一套將AI變成「公司可用方案」嘅方法。' : i === 1 ? '佢會話：「搵外面顧問做啦」。Big Four入場，McKinsey入場，幾百萬項目開始。佢哋唔識你公司、唔識你流程、唔識你團隊。' : '幾個月後——做咗一堆系統、寫咗一堆流程，但用唔落。AI轉型失敗，而你開始被懷疑。其實最有資格做AI轉型嘅人，一直都係你。'}
                            </p>
                        </div>
                    ))}
                </div>
    ...
    return (
        <section id="solution" className="sec-rev" style={{ padding: '8rem 5vw', background: T.bg }}>
            <div style={{ maxWidth: 1000, margin: '0 auto' }}>
                <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
                    <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', letterSpacing: '0.1em', color: '#059669', marginBottom: '1.5rem', textTransform: 'uppercase' }}>// 解決方案</p>
                    <h2 style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)', fontWeight: 600, letterSpacing: '-0.03em', lineHeight: 1.1 }}>AI中層晉升課程</h2>
                    <p style={{ fontSize: '1.25rem', color: T.muted, marginTop: '1rem', maxWidth: 700, margin: '1rem auto 0' }}>
                        唔係教你用AI工具，係教你：<br />👉 設計一套AI工作流<br />👉 建立企業級應用方案<br />👉 用AI直接影響決策
                    </p>
                </div>
                <div style={{ marginTop: '4rem' }}>
                    <ul className="grid grid-cols-1 grid-rows-none gap-4 md:grid-cols-12 md:grid-rows-2 lg:gap-4 xl:max-h-[34rem] xl:grid-rows-2">
                        {[{ icon: <Target className="h-4 w-4" />, title: '工具 → 策略', desc: '你會理解：AI點樣幫公司賺錢、慳成本、提升效率。從操作者變成策略設計者。', area: 'md:[grid-area:1/1/2/7] xl:[grid-area:1/1/2/7]' },
                          { icon: <Settings className="h-4 w-4" />, title: '執行 → 設計', desc: '你會建立：Marketing AI流程、文件與報告自動化、數據分析與決策框架。', area: 'md:[grid-area:1/7/2/13] xl:[grid-area:1/7/2/13]' },
                          { icon: <TrendingUp className="h-4 w-4" />, title: '員工 → 被看見', desc: '你會完成一個真實AI MVP項目，用成果講價，成為公司離唔開嘅人。', area: 'md:[grid-area:2/1/3/7] xl:[grid-area:2/1/3/7]' },
                          { icon: <Users className="h-4 w-4" />, title: '真實CEO展示', desc: 'Session 7：你會站上台，面對真正CEO/MD，做一場畢業展示。唔係模擬，係真實。', area: 'md:[grid-area:2/7/3/13] xl:[grid-area:2/7/3/13]' }].map((item, i) => (
        ...
        <section id="outcomes" className="sec-rev" style={{ padding: '8rem 5vw', background: T.surface }}>
            <div style={{ maxWidth: 1200, margin: '0 auto' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '4rem', flexWrap: 'wrap', gap: '1rem' }}>
                    <div>
                        <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', letterSpacing: '0.1em', color: '#059669', marginBottom: '1rem', textTransform: 'uppercase' }}>// 你會得到咩</p>
                        <h2 style={{ fontSize: '2.5rem', fontWeight: 600, letterSpacing: '-0.03em', margin: 0 }}>7堂課，三個轉變</h2>
                    </div>
...
function CurriculumOverview() {
    return (
        <section id="curriculum" className="sec-rev" style={{ padding: '8rem 5vw', background: T.bg }}>
            <div style={{ maxWidth: 1200, margin: '0 auto' }}>
                <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
                    <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', letterSpacing: '0.1em', color: '#059669', marginBottom: '1.5rem', textTransform: 'uppercase' }}>// 課程大綱</p>
                    <h2 style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)', fontWeight: 600, letterSpacing: '-0.03em', lineHeight: 1.1 }}>From Operator to Builder</h2>
                    <p style={{ fontSize: '1.1rem', color: T.muted, marginTop: '1rem' }}>從「諗」到「做」，再到「展示」</p>
                </div>
                        ...
                        <div key={i} className="h-full">
                            <div className="group h-full relative flex flex-col rounded-[1.5rem] bg-white border border-[rgba(0,0,0,0.08)] p-8 md:p-10 transition-all duration-300 ease-out hover:-translate-y-1.5 hover:shadow-[0_16px_40px_rgba(0,0,0,0.06)]">
                                ...
                                <div style={{ flexGrow: 1 }}>
                                    <h5 style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.7rem', letterSpacing: '0.05em', color: T.text, marginBottom: '0.75rem', textTransform: 'uppercase', fontWeight: 600 }}>學習內容</h5>
                                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                        {c.learn.map((t, idx) => (
                                            <li key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', fontFamily: 'Manrope, sans-serif', color: T.muted, fontSize: '0.92rem', lineHeight: 1.45 }}>
                                                <div style={{ minWidth: '4px', height: '4px', borderRadius: '50%', background: T.muted, marginTop: '0.55rem' }} /><span>{t}</span>
                                            </li>
                                        ))}
                                    </ul>
                                </div>
    ...
    return (
        <section id="pricing" className="sec-rev" style={{ padding: '8rem 5vw', background: T.surface }}>
            <div style={{ maxWidth: 600, margin: '0 auto', textAlign: 'center' }}>
                <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', letterSpacing: '0.1em', color: '#EF4444', marginBottom: '1rem', textTransform: 'uppercase' }}>// 限時Offer 🔥</p>
                <h2 style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 600, letterSpacing: '-0.03em', marginBottom: '2rem' }}>立即鎖定你嘅位置</h2>
                <div style={{ padding: '3rem', background: T.bg, borderRadius: '1.5rem', border: `2px solid #EF4444`, boxShadow: '0 10px 40px rgba(239,68,68,0.1)' }}>
                    <div style={{ textDecoration: 'line-through', color: T.muted, fontSize: '1.5rem', marginBottom: '0.5rem' }}>原價：$15,000</div>
                    <div style={{ fontSize: '3.5rem', fontWeight: 700, color: '#EF4444', marginBottom: '0.5rem' }}>$2,000</div>
                    <div style={{ color: T.muted, marginBottom: '2rem' }}>只收 10 位學員 · 滿額即止</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', textAlign: 'left', marginBottom: '2rem' }}>
                        {['7堂實戰課程', '真實CEO/MD展示機會', '個人AI MVP項目指導', '課後社群支援'].map((item, i) => (
                    ...
                    <button style={{ width: '100%', background: '#EF4444', color: '#FFF', padding: '1.25rem', border: 'none', borderRadius: '1rem', fontSize: '1.1rem', fontWeight: 600, cursor: 'pointer' }}>立即報名</button>
                </div>
            </div>
        </section>
    )
    ...
    return (
        <section id="for-who" className="sec-rev" style={{ padding: '8rem 5vw', background: T.bg }}>
            <div style={{ maxWidth: 1280, margin: '0 auto', textAlign: 'center' }}>
                <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', letterSpacing: '0.1em', color: '#059669', marginBottom: '1rem', textTransform: 'uppercase' }}>// 適合對象</p>
                <h2 style={{ fontSize: '2.5rem', fontWeight: 600, letterSpacing: '-0.03em', marginBottom: '3rem' }}>Who is this for?</h2>
                <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '1rem', maxWidth: 800 }}>
                    {['30–45歲中層／中高層', '需要向C-Level或Board匯報', '公司開始做AI，但無方向', '想升職／轉工，但缺成果'].map((p, i) => (
